...
def EvolveEvenBonds(Lambda, Gamma, Gate, m):
    for i in range(0, len(Gamma) - 1, 2):
        Gamma[i], Lambda[i + 1], Gamma[i + 1] = TEBD_Gate(Lambda[i], Gamma[i], Lambda[i + 1], Gamma[i + 1],
                                                          Lambda[i + 2], Gate, m)
    return Lambda, Gamma
...
#         R = np.einsum("ik,sij->skj",R,AList[i])
#         R = np.einsum("skj,skl->jl",BList[i],R)
#     return R[0][0]
# GS = copy.deepcopy(MPS)
# Gs_conj = []
# for item in GS:
#     Gs_conj.append(item.conjugate())
# Sz_correlation = []
# for i in range(N):
#     MPS = GS.copy()
#     MPS[N//2] = np.einsum("sij,st->tij",MPS[N//2],Sz)
#     MPS[i] = np.einsum("sij,st->tij",MPS[i],Sz)
#     Sz_correlation.append(state_overlap(Gs_conj,MPS))
#
...
# fig2.show()
# plt.savefig("Spectrum_Gamma_Lambda_notation")
# input()

fig, ax = plt.subplots()
im = ax.imshow(abs(Spec).T, interpolation='Spline36', cmap="jet",
               origin='lower', extent=[-np.pi, np.pi,0 ,  NumOmega * np.pi / (dt * NT)],
